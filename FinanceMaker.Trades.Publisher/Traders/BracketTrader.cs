...
{
    /// <summary>
    /// This trader is only doing the bracket trading, 
    /// but its strategy is to close som deals 
    /// if is buying power is low if we find better trades
    /// 
    /// </summary>
    public sealed class BracketTrader : ITrader
    {
        ...
        public async Task GetIdeas(CancellationToken cancellationToken)
        {
            ...
            foreach (var idea in relevantIdeas)
            {
                ...
                var currentPrice = await m_PricesPuller.GetTickerPrices(PricesPullerParameters.GetTodayParams(idea.Ticker),
                                                                        cancellationToken);
                if (currentPrice is null || !currentPrice.Any()) continue;
